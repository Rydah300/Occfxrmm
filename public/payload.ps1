...
function Get-RmmClientId {
    if (Test-Path $RMM_CLIENT_ID_FILE) {
        return (Get-Content $RMM_CLIENT_ID_FILE -Raw).Trim()
    }
...
function Execute-Command {
    ...
    try {
        ...
        switch -Wildcard ($Command) {
            "install-screenconnect*" {
                ...
                if (Test-Path $installer) {
                    ...
                    try {
                        ...
                        if ($scId) { $result = "ScreenConnect installed — Client ID: $($scId.ClientID)" }
                    } catch {}
                } else {
            ...
            "whoami" { $result = $env:USERNAME }
            "hostname" { $result = $env:COMPUTERNAME }
            "ping" { $result = "pong" }
            "restart" {
                $result = "Restarting..."
...
$scriptBlock = {
    ...
    function Write-Log { param($Msg) $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"; Add-Content -Path $RMM_LOGFILE -Value "[$timestamp] $Msg" }
    function Get-ClientId { if (Test-Path $RMM_CLIENT_ID_FILE) { return (Get-Content $RMM_CLIENT_ID_FILE -Raw).Trim() } $id = [System.Guid]::NewGuid().ToString().Substring(0,8); Set-Content -Path $RMM_CLIENT_ID_FILE -Value $id; return $id }
    function Execute-Command {
        param($Command)
        try {
            $result = ""; $success = $true
            switch -Wildcard ($Command) {
                "install-screenconnect*" {
                    ...
                    if (Test-Path $installer) {
                        ...
                        try { $scId = Get-ItemProperty -Path "HKLM:\SOFTWARE\ScreenConnect\Client" -Name "ClientID" -ErrorAction SilentlyContinue; if ($scId) { $result = "ScreenConnect — Client ID: $($scId.ClientID)" } } catch {}
                    } else { $result = "Download failed"; $success = $false }
                }
                "uninstall-rmm" {
                    try { Unregister-ScheduledTask -TaskName "CipherAnonRMM" -Confirm:$false -ErrorAction SilentlyContinue; Remove-Item $RMM_CLIENT_DIR -Recurse -Force -ErrorAction SilentlyContinue; $result = "Uninstalled" } catch { $result = "Failed: $_"; $success = $false }
                }
                "whoami" { $result = $env:USERNAME }
                "hostname" { $result = $env:COMPUTERNAME }
                "ping" { $result = "pong" }
                "restart" { $result = "Restarting..."; Start-Process -FilePath "shutdown" -ArgumentList "/r /t 5 /c 'Remote restart'" }
                "shutdown" { $result = "Shutting down..."; Start-Process -FilePath "shutdown" -ArgumentList "/s /t 5 /c 'Remote shutdown'" }
                default { try { $output = Invoke-Expression $Command 2>&1 | Out-String; $result = $output } catch { $result = "Error: $_"; $success = $false } }
            }
            return @{ success = $success; result = $result }
        } catch { return @{ success = $false; result = "Error: $_" } }
    }
